...
$("#frmTodo").submit(function (event) {
    ...
    var tarih = new Date()
    var date = tarih.getDate() + "." + (tarih.getMonth() + 1) + "." + tarih.getFullYear() + "-" + tarih.getHours() + ":" + tarih.getMinutes() + ":" + tarih.getSeconds();
...
function listTodos() {
    ...
    $.each(sortedTodos, function (index, todo) {
        ...
        li.append(cb);
        li.append(" " + todo.task);
        li.append("<br/>" + "<p style='font-size:10px;color:gray;font-family:roboto;'>" + todo.datetime + "</p>")
        $("ul#todos").append(li);
    });
}
...
function yildizat(id) {
    ...
    $("#yildiz").prop('checked', false);
    saveData();
    listTodos();
}
$("#yildiz").change(function () {
    if ($(this).is(':checked')) {
        todosdate.splice(0, todosdate.length);
